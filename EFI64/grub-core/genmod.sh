...
if test x0 != x1; then
    ...
    if test -n "$deps"; then
	gcc  -m64 -Wl,-melf_x86_64 -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $t2 $tmpfile -Wl,-r
    else
	gcc  -m64 -Wl,-melf_x86_64 -no-pie -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $tmpfile -Wl,-r
    fi
    rm -f $t1 $t2 $tmpfile
    mv $tmpfile2 $tmpfile
